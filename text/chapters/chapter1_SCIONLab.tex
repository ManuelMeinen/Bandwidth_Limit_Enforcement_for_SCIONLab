...
\\
\begin{enumerate}
\item A \textbf{\acl{BS}} that sends and receives the \acsp{PCB}
\item A \textbf{\acl{PS}} that stores the path segments and disseminates them to the customers.
\item A \textbf{\acl{CS}} that holds the certificates which are used to validate the paths.
\item A \textbf{\acl{BR}} that routes (on a \acs{SCION}-level) the traffic leaving the \acs{AS}.
\end{enumerate}
